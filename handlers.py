...
from config import (
    ...
    MEDIA_DIR, MAX_COOLDOWN_ENTRIES, MAX_FILE_SIZE_MB, logger,
)
from core import (
    state, db, command_cooldown, reply_cooldown, owner_filter,
    respond, _get_translator, _download_lock as core_dl_lock,
    _watch_task as core_wt, _protect_task as core_pt,
)
from downloaders import run_download, send_and_clean, set_max_file_size
from rp_commands import RP_COMMANDS, format_rp_action, get_rp_reply
from utils import fmt_time, progress_bar, safe_eval, caesar, morse_enc, gen_pwd, vigenere

_download_lock = core_dl_lock or asyncio.Lock()
_watch_task = core_wt
_protect_task = core_pt
...
async def spam_cmd(e):
    ...
    now = time.time()
    if command_cooldown.get(cooldown_key, 0) > now:
        remaining = int(command_cooldown[cooldown_key] - now)
        await respond(e, f"❌ Подожди {remaining} сек.")
        return
    command_cooldown[cooldown_key] = now + 60
    await respond(e, f"✉️ Отправляю **{count}** сообщений...")
    for i in range(count):
        try:
            await client.send_message(e.chat_id, text)
            await asyncio.sleep(0.3)
        except FloodWaitError as ex:
            logger.warning(f"FloodWait: {ex.seconds}s")
            await asyncio.sleep(ex.seconds)
        except Exception as ex:
            logger.error(f"Spam error: {ex}")
            break
    db.bump_stat('cmds')
...
async def forward_cmd(e):
    ...
    if not e.reply_to_msg_id:
        await respond(e, "ℹ️ Ответь на сообщение + `!forward [chat_id]`")
        return
    msg = await e.get_reply_message()
    try:
        await client.send_message(target, msg)
        await respond(e, f"✅ Сообщение переслано в `{target}`")
    except Exception as ex:
...
async def pin_cmd(e):
    if not e.reply_to_msg_id:
        await respond(e, "ℹ️ Ответь на сообщение → `!pin`")
        return
    r = await e.get_reply_message()
    try:
        await client.pin_message(e.chat_id, r.id)
        await respond(e, "📌 Сообщение закреплено.")
    except Exception as ex:
        await respond(e, f"❌ {ex}")
    db.bump_stat('cmds')
...
async def unpin_cmd(e):
    try:
        await client.pin_message(e.chat_id, 0)
        await respond(e, "📌 Закреп удалён.")
    except Exception as ex:
        await respond(e, f"❌ {ex}")
    db.bump_stat('cmds')
...
async def copyall_cmd(e):
    ...
    if count > 100:
        await respond(e, "❌ Максимум 100.")
        return
    msg = await respond(e, f"📋 Копирую **{count}** сообщений...")
    copied = 0
    async for m in client.iter_messages(e.chat_id, limit=count):
        try:
            ...
            continue
    await msg.edit(f"✅ Скопировано **{copied}** сообщений в `{target}`.")
    db.bump_stat('cmds')
...
async def react_cmd(e):
    ...
    if not e.reply_to_msg_id:
        await respond(e, "ℹ️ Ответь на сообщение → `!react 👍`")
        return
    r = await e.get_reply_message()
    try:
        await client(SendReactionRequest(
            peer=await e.get_chat(),
            msg_id=r.id,
            reaction=[ReactionEmoji(emoticon=emoji)]
        ))
        await respond(e, f"✅ Реакция {emoji} поставлена.")
    except Exception as ex:
        await respond(e, f"❌ {ex}")
    db.bump_stat('cmds')
...
async def save_media_cmd(e):
    if not e.reply_to_msg_id:
        await respond(e, "ℹ️ Ответь на медиа → `!save`")
        db.bump_stat('cmds')
        return
    r = await e.get_reply_message()
    if not r.file:
        await respond(e, "❌ Это не медиафайл.")
        db.bump_stat('cmds')
        return
    os.makedirs(MEDIA_DIR, exist_ok=True)
    path = await r.download_media(MEDIA_DIR)
    if path:
        db.set_saved(os.path.basename(path), path)
        await respond(e, f"✅ Медиа сохранено как `{os.path.basename(path)}`")
    else:
        await respond(e, "❌ Ошибка загрузки медиа.")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!save (\S+) (.+)', func=owner_filter))
async def save_cmd(e):
    k, v = e.pattern_match.group(1), e.pattern_match.group(2).strip()
    db.set_saved(k, v)
    await respond(e, f"✅ **{k}** → сохранено.")
    db.bump_stat('cmds')
...
async def get_cmd(e):
    ...
    v = db.get_saved(k)
    if v is not None and os.path.exists(v) and not v.startswith('http'):
        try:
            await e.reply(file=v)
            return
        except Exception:
            pass
    await respond(e, f"📂 **{k}**\n\n{v or '❌ Не найдено.'}")
...
async def del_cmd(e):
    k = e.pattern_match.group(1)
    db.del_saved(k)
    await respond(e, f"🗑️ **{k}** удалено.")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!list$', func=owner_filter))
async def list_cmd(e):
    all_saved = db.all_saved()
    if not all_saved:
        await respond(e, "📭 База данных пуста.")
    else:
        out = "\n".join(f"• `{k}`" for k in all_saved)
        await respond(e, f"📂 **Сохранённые данные ({len(all_saved)}):**\n\n{out}")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!find (.+)', func=owner_filter))
async def find_cmd(e):
    q = e.pattern_match.group(1).strip()
    rows = db.search_saved(q)
    if not rows:
        await respond(e, f"❌ Ничего не найдено по «{q}».")
    else:
        out = "\n".join(f"• **{r[0]}**: `{r[1][:80]}{'…' if len(r[1]) > 80 else ''}`" for r in rows[:20])
        await respond(e, f"🔍 **Результаты по «{q}»:**\n\n{out}")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!note (\S+)(?: (.+))?', func=owner_filter))
async def note_cmd(e):
    k = e.pattern_match.group(1)
    v = (e.pattern_match.group(2) or '').strip()
    if not v:
        await respond(e, "ℹ️ `!note <название> <текст>`")
        return
    db.set_note(k, v)
    await respond(e, f"📝 Заметка **{k}** сохранена.")
    db.bump_stat('cmds')
...
async def delnote_cmd(e):
    k = e.pattern_match.group(1)
    db.del_note(k)
    await respond(e, f"🗑️ Заметка **{k}** удалена.")
    db.bump_stat('cmds')
...
async def notes_cmd(e):
    ...
    if not notes:
        ...
        await respond(e, f"📝 **Заметки ({len(notes)}):**\n\n{out}")
    db.bump_stat('cmds')
...
async def todo_add_cmd(e):
    ...
    await respond(e, f"✅ Задача **#{tid}** добавлена.\n📌 _{text}_")
    db.bump_stat('cmds')
...
async def todos_cmd(e):
    todos = db.get_todos()
    if not todos:
        await respond(e, "✅ Нет задач.")
        return
    lines = [f"📋 **Задачи ({len(todos)}):**\n"]
    for t in todos:
        status = "✅" if t['done'] else "⏳"
        lines.append(f"{status} `#{t['id']}` {t['text'][:60]}")
    await respond(e, "\n".join(lines))
    db.bump_stat('cmds')
...
async def done_cmd(e):
    ...
    await respond(e, f"✅ Задача `#{tid}` отмечена как выполненная.")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!undone (\d+)', func=owner_filter))
async def undone_cmd(e):
    tid = int(e.pattern_match.group(1))
    db.update_todo(tid, False)
    await respond(e, f"🔄 Задача `#{tid}` снова активна.")
    db.bump_stat('cmds')
...
async def deltodo_cmd(e):
    tid = int(e.pattern_match.group(1))
    db.del_todo(tid)
    await respond(e, f"🗑️ Задача `#{tid}` удалена.")
    db.bump_stat('cmds')
...
async def afk_cmd(e):
    ...
    await respond(e, f"😴 **AFK ВКЛЮЧЁН**{' — ' + reason if reason else ''}.")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!unafk$', func=owner_filter))
async def unafk_cmd(e):
    duration = state.clear_afk()
    d_str = fmt_time(int(duration)) if duration else "неизвестно"
    await respond(e, f"👋 **AFK ВЫКЛЮЧЕН.** Отсутствовал: {d_str}.")
    db.bump_stat('cmds')
...
async def chatinfo_cmd(e):
    chat = await e.get_chat()
    title = getattr(chat, 'title', 'ЛС') or 'ЛС'
    username = f"@{chat.username}" if getattr(chat, 'username', None) else "нет"
    try:
        members = await client.get_participants(chat, limit=0)
        member_count = members.total if hasattr(members, 'total') else len(members)
    except Exception:
        member_count = '?'
    await respond(e, 
        ...
        f"🔒 Приватный: {'✅' if getattr(chat, 'username', None) is None else '❌'}"
    )
    db.bump_stat('cmds')
...
async def members_cmd(e):
    try:
        ...
        total = participants.total if hasattr(participants, 'total') else len(participants)
        await respond(e, f"👥 Участников в чате: **{total}**")
    except Exception as ex:
        await respond(e, f"❌ {ex}")
    db.bump_stat('cmds')
...
async def top_cmd(e):
    ...
    try:
        participants = await client.get_participants(e.chat_id)
        top_users = participants[:n]
        lines = [f"🏆 **Топ {n} пользователей:**\n"]
        for i, u in enumerate(top_users, 1):
            name = f"{u.first_name or ''} {u.last_name or ''}".strip() or 'Deleted'
...
async def bots_cmd(e):
    try:
        ...
        if not bots:
            await respond(e, "🤖 В этом чате нет ботов.")
            return
        lines = [f"🤖 **Боты ({len(bots)}):**\n"]
        for b in bots:
            name = f"{b.first_name or ''} {b.last_name or ''}".strip() or '?'
            lines.append(f"• {name}" + (f" @{b.username}" if b.username else ''))
        await respond(e, "\n".join(lines))
    except Exception as ex:
        await respond(e, f"❌ {ex}")
    db.bump_stat('cmds')
...
async def resetdata_cmd(e):
    await respond(e, "⚠️ **Точно удалить ВСЕ данные?** Напиши `!resetconfirm`")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!resetconfirm$', func=owner_filter))
async def resetconfirm_cmd(e):
    db.clear_all()
    state._load()
    await respond(e, "🗑️ **Все данные удалены.**")
    db.bump_stat('cmds')

# ─── ЗАГРУЗКА МЕДИА ───────────────────────────────────────────

@client.on(events.NewMessage(pattern=r'!ytshow\s+(\S+)(?:\s+(\d+))?$', func=owner_filter))
async def ytshow_cmd(e):
    global _download_lock
    url = e.pattern_match.group(1)
    quality = int(e.pattern_match.group(2)) if e.pattern_match.group(2) else None
    msg = await respond(e, "⏳ Начинаю загрузку...")
    async with _download_lock:
        filename = await run_download(
            lambda t: msg.edit(t),
            url, mode='video', quality=quality
        )
        await send_and_clean(lambda t: msg.edit(t), client, e.chat_id, filename)
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!dl\s+(\S+)', func=owner_filter))
async def dl_cmd(e):
    global _download_lock
    url = e.pattern_match.group(1)
    msg = await respond(e, "⏳ Начинаю загрузку...")
    async with _download_lock:
        filename = await run_download(
            lambda t: msg.edit(t), url, mode='video'
        )
        await send_and_clean(lambda t: msg.edit(t), client, e.chat_id, filename)
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!playlist\s+(\S+)(?:\s+(.+))?$', func=owner_filter))
async def playlist_cmd(e):
    global _download_lock
    url = e.pattern_match.group(1)
    extra = e.pattern_match.group(2)
    async with _download_lock:
        msg = await respond(e, "⏳ Загружаю плейлист...")
        try:
            pl_opts = {
                'outtmpl': os.path.join(MEDIA_DIR, '%(playlist_title)s/%(playlist_index)s - %(title)s.%(ext)s'),
            ...
            if extra:
                try:
                    ...
                    pl_opts['playliststart'] = int(start)
                    pl_opts['playlistend'] = int(end)
                except ValueError:
                    pl_opts['playlistend'] = int(extra)
            import yt_dlp
            def _dl():
                with yt_dlp.YoutubeDL(pl_opts) as ydl:
                    ydl.download([url])
            await asyncio.to_thread(_dl)
            await msg.edit("✅ Плейлист загружен в папку media/")
        except Exception as ex:
            await msg.edit(f"❌ Ошибка: {ex}")
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!audio\s+(\S+)', func=owner_filter))
async def audio_cmd(e):
    global _download_lock
    url = e.pattern_match.group(1)
    msg = await respond(e, "⏳ Загружаю аудио...")
    async with _download_lock:
        filename = await run_download(
            lambda t: msg.edit(t), url, mode='audio'
        )
        await send_and_clean(lambda t: msg.edit(t), client, e.chat_id, filename)
    db.bump_stat('cmds')

@client.on(events.NewMessage(pattern=r'!sub\s+(\S+)(?:\s+(\w+))?$', func=owner_filter))
async def sub_cmd(e):
    url = e.pattern_match.group(1)
    lang = e.pattern_match.group(2) or 'ru'
    msg = await respond(e, "⏳ Загружаю субтитры...")
    try:
        import yt_dlp
        sub_opts = {
            'skip_download': True,
        ...
        def _dl():
            ...
            for f in os.listdir(sub_dir):
                if f.endswith(f'{lang}.vtt') or f.endswith(f'{lang}.srt'):
                    return os.path.join(sub_dir, f)
            return None
        sub_path = await asyncio.to_thread(_dl)
        if sub_path:
            await client.send_file(e.chat_id, sub_path, caption=f"📜 Субтитры ({lang})")
            os.remove(sub_path)
            await msg.delete()
        else:
            await msg.edit(f"❌ Субтитры ({lang}) не найдены.")
    except Exception as ex:
        await msg.edit(f"❌ Ошибка: {ex}")
    db.bump_stat('cmds')
        ...
        async def monitor():
            while True:
                try:
                    ...
                    existing = {d.id for d in dialogs}
                    protected = set(db.get_protected_chat_ids())
                    for pid in protected:
                        if pid not in existing:
                            me = await client.get_me()
                            await client.send_message(me.id, f"⚠️ **Вас удалили из чата** `{pid}`!")
                            db.del_protected_chat(pid)
                            logger.warning(f"Removed from chat {pid}")
                    for d in dialogs:
                        pid = d.id
                        if pid not in protected:
                            db.add_protected_chat(pid)
                except Exception as ex:
                    logger.error(f"Protect error: {ex}")
                await asyncio.sleep(120)
        _protect_task = asyncio.create_task(monitor())
        await respond(e, "🛡️ **Защита чатов ВКЛЮЧЕНА.**")
    else:
        if _protect_task and not _protect_task.done():
            _protect_task.cancel()
            _protect_task = None
        await respond(e, "🛡️ **Защита чатов ВЫКЛЮЧЕНА.**")
    db.bump_stat('cmds')

# ─── RP ────────────────────────────────────────────────────────

@client.on(events.NewMessage(pattern=r'^!rphelp$', func=owner_filter))
async def rphelp_cmd(e):
    from rp_commands import get_all_categories, get_category_commands
    text = "🎭 **RP-команды**\n\n"
    for cat in get_all_categories():
        cmds = get_category_commands(cat)
        text += f"**{cat.capitalize()}:** {', '.join(cmds)}\n"
    text += "\n💡 Ответьте на сообщение с командой + Enter + свой текст"
    await respond(e, text)
    db.bump_stat('cmds')


# ─── НЕ-КОМАНДНЫЕ ХЕНДЛЕРЫ ─────────────────────────────────────

@client.on(events.NewMessage(func=lambda e: e.is_private and not owner_filter(e)))
async def private_msg_handler(event):
    sender = await event.get_sender()
    uid = event.sender_id
    text = event.raw_text or ''
    name = getattr(sender, 'first_name', 'пользователь') or 'пользователь'

    # Логирование
    logger.info(f"ЛС от {name} ({uid}): {len(text)} симв.")

    # AFK
    if state.afk_start_time:
        await asyncio.sleep(1)
        await event.reply(f"😴 **AFK:** {state.afk_reason or 'нет причины'}")

    # Read receipt
    if state.readreceipt_enabled and event.is_private:
        try:
    ...
    if state.typing_enabled:
        try:
            async with client.action(event.chat_id, 'typing'):
                await asyncio.sleep(state.reply_delay or 0.5)
        except Exception:
            pass
    ...
    # Shadow — ответ на время → удалить
    sent_msg = None
    if state.shadow_enabled:
        try:
            sent_msg = await event.reply(state.auto_reply_text)
        except Exception:
            pass
        if sent_msg:
            await asyncio.sleep(state.shadow_delay)
            try:
    ...
    # Silence — без ответа, только прочтение
    if state.silent_enabled:
        return
    ...
    if state.auto_reply_enabled:
        auto_text = state.auto_reply_text
        custom = db.get_reply_text(str(uid))
        if custom:
            auto_text = custom
        default = db.get_default_reply()
        if default:
            auto_text = default
        try:
...
@client.on(events.MessageEdited(func=lambda e: e.is_private and not owner_filter(e)))
async def pm_edit_handler(event):
    text = event.raw_text or ''
    sender = await event.get_sender()
    name = getattr(sender, 'first_name', 'пользователь') or 'пользователь'
    logger.info(f"✏️ {name} отредактировал(а): {len(text)} симв.")


@client.on(events.NewMessage(pattern=r'^\S+$', func=owner_filter))
async def rp_reply_handler(event):
    if not event.is_reply:
        return
    action_key = event.raw_text.strip().lower()
    if action_key not in RP_COMMANDS:
        return
    replied = await event.get_reply_message()
    reply_sender = await replied.get_sender()
    target_name = getattr(reply_sender, 'first_name', 'пользователь') or 'пользователь'
    if replied.sender_id == OWNER_ID or replied.sender_id in state.sudo_users:
        target_name = 'себя'

    action_text = format_rp_action(action_key, 'я', target_name)
    if action_text:
        await event.delete()
        rp_msg = action_text + '\n\n'
        if replied.text:
            rp_msg += f'_{replied.text}_'
        await replied.reply(rp_msg)
